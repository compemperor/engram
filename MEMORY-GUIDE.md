...
**5. Learning Session PROPER FLOW**
- Start with CORRECT params: `POST /learning/session/start?topic=X&duration_min=N`
  - Parameter is `duration_min` NOT `planned_duration_min`
- Add notes during research with `source_quality` ratings
- **VERIFY = actually cross-reference against multiple primary sources**
  - Only call `/verify` with `sources_verified: true` if you ACTUALLY verified
  - Finding same info on 2+ independent sources = verified
  - Single source or unconfirmed = NOT verified
- Use FULL duration requested - finishing early means you didn't dig deep enough
- Call `/verify` checkpoints when you genuinely confirm information
- Consolidate at end
- If consolidation gives `source_quality: 5.0` → you didn't do real verification, accept it

**6. Verification is NOT a checkbox**
- The `/verify` endpoint checks if you cross-referenced sources
- `sources_verified: false` → consolidation quality = 5.0 (rejected)
- `sources_verified: true` → consolidation quality = 9.0 (accepted if understanding is high)
- Don't lie to pass consolidation - that defeats the purpose
- Individual note `source_quality` ratings are IGNORED by consolidation
- Only `sources_verified` from `/verify` calls matters
