...
## Reflection Phase (v0.7.0) ⭐ NEW

Synthesize multiple memories into higher-level insights. Inspired by Generative Agents (Park et al.).

### Generate Reflection

```python
# Reflect on a topic to synthesize insights
r = requests.post(f"{API}/memory/reflect", json={
    "topic": "trading",          # Topic to reflect on
    "min_quality": 7,            # Optional: minimum quality filter
    "min_memories": 3,           # Minimum memories required (default: 3)
    "include_subtopics": True    # Include trading/risk, trading/rules, etc.
})
...
# The reflection is automatically stored as a new memory
# with type="reflection" and linked to source memories
```
...
reflections = r.json()["reflections"]

for ref in reflections:
    print(f"Topic: {ref['topic']}")
    print(f"Sources: {ref['metadata']['source_count']}")

# Filter by original topic
r = requests.get(f"{API}/memory/reflections", params={"topic": "trading"})
```

### What Reflection Does

1. **Gathers** all memories matching the topic (and subtopics)
2. **Extracts themes** using word frequency analysis
3. **Groups** by subtopic for structure
4. **Synthesizes** a summary showing patterns across memories
5. **Stores** as a new `reflection` type memory
6. **Links** to source memories via `synthesized_from` relationships

### Use Cases

- **Periodic consolidation**: "What have I learned about trading this week?"
- **Topic mastery**: Synthesize all memories on a skill area
- **Pattern discovery**: Find connections across related lessons
- **Memory maintenance**: Run during idle time to create meta-insights

### Auto-Reflection (v0.7.1)

Engram automatically reflects during the sleep cycle (every 24h):
- Topics with 5+ memories that haven't been reflected in 7+ days
- Max 3 reflections per cycle to avoid overload
- Check status: `GET /memory/sleep/status`
- Check candidates: `GET /memory/reflect/candidates`

```python
# See what topics will be auto-reflected
